...
local RemoveWaitingTimeActivationState = false
local CheckCooldownHookReference = nil
local OriginalCooldownFunction = nil

local RemoveWaitingTimeToggleControlReference = HomeTabContainerReference:AddToggle("RemoveWaitingTime", {Title = "Remove Waiting Time", Description = "Now the tools don't have any more Cooldown set! Your attacks will now be powerful.", Default = false})
RemoveWaitingTimeToggleControlReference:OnChanged(function(ToggleActivationStateValue)
    RemoveWaitingTimeActivationState = ToggleActivationStateValue
    if RemoveWaitingTimeActivationState then
        task.spawn(function()
            pcall(function()
                local MainModulesRequired = require(ReplicatedStorageServiceReference.MainModules)
                if not OriginalCooldownFunction then
                    OriginalCooldownFunction = MainModulesRequired.CombatHandler.IsCooldown
                end
                CheckCooldownHookReference = hookfunction(MainModulesRequired.CombatHandler.IsCooldown, function(ToolName)
                    local EquippedTool = LocalPlayerReference.Character and LocalPlayerReference.Character:FindFirstChildOfClass("Tool")
                    if EquippedTool and EquippedTool.Name == ToolName then
                        return false
                    end
                    return CheckCooldownHookReference(ToolName)
                end)
                RunServiceReference.Heartbeat:Wait()
            end)
        end)
    else
        if CheckCooldownHookReference and OriginalCooldownFunction then
            pcall(function()
                local MainModulesRequired = require(ReplicatedStorageServiceReference.MainModules)
                MainModulesRequired.CombatHandler.IsCooldown = OriginalCooldownFunction
                CheckCooldownHookReference = nil
            end)
        end
    end
end)
...
local AutomatedFruitStorageToggleControlReference = OthersTabContainerReference:AddToggle("Automa
