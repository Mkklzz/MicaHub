...
local QuestSystemModule = loadstring(game:HttpGet("https://raw.githubusercontent.com/Mkklzz/MicaHub/Home/LoadsEssentials/VoxSeasQuests.lua"))()
getgenv().MicaHubQuestSystem = QuestSystemModule

local BetweenSidesRemoteConnectionFolder = ReplicatedStorageService:WaitForChild("BetweenSides")
local RemoteEventsCommunicationFolder = BetweenSidesRemoteConnectionFolder:WaitForChild("Remotes")
local GameEventHandlersContainer = RemoteEventsCommunicationFolder:WaitForChild("Events")
local DialogueSystemRemoteEventHandler = GameEventHandlersContainer:WaitForChild("DialogueEvent")
local QuestManagementRemoteEventHandler = GameEventHandlersContainer:WaitForChild("QuestEvent")
...
local AuthorTabContainer, HomeTabContainer, StatusTabContainer, OthersTabContainer, SettingsTabContainer = PrimaryDashboardWindow:AddTab({ Title = "Author", Icon = "user" }), PrimaryDashboardWindow:AddTab({ Title = "Home", Icon = "home" }), PrimaryDashboardWindow:AddTab({ Title = "Status", Icon = "trending-up" }), PrimaryDashboardWindow:AddTab({ Title = "Others", Icon = "layers" }), PrimaryDashboardWindow:AddTab({ Title = "Settings", Icon = "settings" })

AuthorTabContainer:AddSection("Author Information")
AuthorTabContainer:AddParagraph({Title = "Thank you for using my Dashboard!", Content = "I'm still developing, some functions may be incomplete or bugs!"})
AuthorTabContainer:AddParagraph({Title = "GitHub: Mkklzz", Content = ""})

AuthorTabContainer:AddButton({Title = "Join Discord Server", Description = "Join the author's discord. To receive information or updates", Callback = function()
    PrimaryDashboardWindow:Dialog({Title = "Discord link copied", Content = "The Discord link has been copied to your clipboard. You can now paste it in your browser to join the server.", Buttons = {
...
local ToolSelectionForFarmingDropdown = HomeTabContainer:AddDropdown("ToolSelectionForFarming", {Title = "Select Tool Farm", Values = {"BlackLeg", "Combat", "Eletric", "WaterKungFu"}, Multi = false, Default = "Combat"})
...
local function ExecuteAutomatedQuestFarmingRoutine()
    ...
    if SelectedToolName and SelectedToolName ~= "" then
        ...
        if not EquippedTool then
            local AvailableToolsList = {"BlackLeg", "Combat", "Eletric", "WaterKungFu"}
            for _, ToolName in pairs(AvailableToolsList) do
                EquippedTool = LocalPlayerReference.Backpack:FindFirstChild(ToolName) or LocalPlayerReference.Character:FindFirstChild(ToolName)
            ...
            if not EquippedTool then
                for _, ToolContainerObject in pairs({LocalPlayerReference.Backpack, LocalPlayerReference.Character}) do
                    for _, IndividualToolObject in pairs(ToolContainerObject:GetChildren()) do
                        if IndividualToolObject:IsA("Tool") then
                            for _, ToolName in pairs(AvailableToolsList) do
                                if IndividualToolObject.Name:find(ToolName) then
                                    EquippedTool = IndividualToolObject
                                    break
                                end
                            end
                            if EquippedTool then break end
        ...
        if EquippedTool and EquippedTool:IsA("Tool") and EquippedTool.Parent == LocalPlayerReference.Backpack then 
            LocalPlayerReference.Character.Humanoid:EquipTool(EquippedTool) 
        end
    end
...
AutomatedQuestFarmingToggleControl:OnChanged(function(ToggleActivationState) 
    ...
    if AutomatedQuestFarmingEnabled then 
        task.spawn(ExecuteAutomatedQuestFarmingRoutine)
        if getgenv().MicaHubQuestSystem and getgenv().MicaHubQuestSystem.StartQuestManagement then
            getgenv().MicaHubQuestSystem.StartQuestManagement()
        end
    else
        if getgenv().MicaHubQuestSystem and getgenv().MicaHubQuestSystem.StopQuestManagement then
            getgenv().MicaHubQuestSystem.StopQuestManagement()
        end
    end
end)
...
local function RetrieveAvailableCharacterTools()
    ...
    local ValidToolNames = {"BlackLeg", "Combat", "Eletric", "WaterKungFu"}
    
    for _, ToolContainerObject in pairs({LocalPlayerReference.Backpack, LocalPlayerReference.Character}) do
        for _, IndividualToolObject in pairs(ToolContainerObject:GetChildren()) do
            if IndividualToolObject:IsA("Tool") then
                for _, ValidToolName in pairs(ValidToolNames) do
                    if IndividualToolObject.Name:find(ValidToolName) then
                        local alreadyExists = false
                        for _, ExistingTool in pairs(AvailableToolsList) do
                            if ExistingTool == IndividualToolObject.Name then
                                alreadyExists = true
                                break
                            end
                        end
...
local BasicSettingsToolDropdown = HomeTabContainer:AddDropdown("BasicSettingsToolSelection", {Title = "Select Tool Farm", Values = {"None Tools"}, Multi = false, Default = "None Tools"})

HomeTabContainer:AddButton({Title = "Update Tools List", Description = "Refresh and update the available tools list with your current inventory", Callback = function()
    local CurrentAvailableToolsList = RetrieveAvailableCharacterTools()
    if #CurrentAvailableToolsList > 0 then
        BasicSettingsToolDropdown:SetValues(CurrentAvailableToolsList)
    end
end})

StatusTabContainer:AddSection("Automatic Status")
...
local function CreateStatusToggle(StatusName, VariableName, RoutineFunction)
    local Toggle = StatusTabContainer:AddToggle("Automated" .. StatusName .. "StatusUpgrade", {Title = "Auto Status " .. StatusName, Default = false})
...
OthersTabContainer:AddSection("Fruits Functionality")
...
local AutomatedFruitCollectionToggleControl = OthersTabContainer:AddToggle("AutomatedFruitCollection", {Title = "Auto Collect Fruits", Default = false })
...
local function ExecuteAutomatedFruitStorageRoutine()
    ...
    for _, ToolContainerObject in pairs({LocalPlayerReference.Backpack, LocalPlayerReference.Character}) do
        for _, IndividualToolObject in pairs(ToolContainerObject:GetChildren()) do
            if IndividualToolObject:IsA("Tool") and IndividualToolObject.Name:match("Fruit$") then
                if LocalPlayerReference.Character.Humanoid then
                    LocalPlayerReference.Character.Humanoid:EquipTool(IndividualToolObject)
...
local AutomatedFruitStorageToggleControl = OthersTabContainer:AddToggle("AutomatedFruitStorage", {Title = "Auto Store Fruits", Default = false })
...
OthersTabContainer:AddSection("Teleporting World")
...
local TeleportationDestinationSelectionDropdown = OthersTabContainer:AddDropdown("TeleportationDestinationSelection", {Title = "Select Teleport Location", Values = RetrieveAvailablePortalLocations(), Multi = false, Default = 1})

OthersTabContainer:AddButton({Title = "Teleport to Location [BETA]", Description = "Teleport to Selected Island", Callback = function()
    if AutomatedQuestFarmingEnabled or AutomatedMobBringingEnabled then
        PrimaryDashboardWindow:Dialog({Title = "MicaHub Information", Content = "Cannot teleport while automated functions are running. Disable Auto Farm Quests and Auto Bring Mobs before teleporting.", Buttons = {
            {Title = "Ok", Callback = function() end}
        }})
        return
    end
...
OthersTabContainer:AddSection("Https Servers Connections")
OthersTabContainer:AddParagraph({Title = "How does it work?", Content = "Join servers with specific items using the code available on our Discord."})

local ServerConnectionCodeInputField = OthersTabContainer:AddInput("ServerConnectionCode", {Title = "Target Server Code", Default = "", Placeholder = "", Numeric = false, Finished = false})

OthersTabContainer:AddButton({Title = "Join the Server", Description = "Use this button to connect to the server", Callback = function()
...
getgenv().MicaHubMainInterface = {
    ToggleStates = {
        QuestFarming = function() return AutomatedQuestFarmingEnabled end,
        MobBringing = function() return AutomatedMobBringingEnabled end,
        FruitCollection = function() return AutomatedFruitCollectionEnabled end,
        FruitStorage = function() return AutomatedFruitStorageEnabled end
    },
    PlayerReferences = {
        LocalPlayer = LocalPlayerReference,
        Character = function() return LocalPlayerReference.Character end,
        PlayerLevel = function() return tonumber(PlayerLevelDisplayElementReference.Text) or 1 end
    },
    RemoteConnections = {
        DialogueEvent = DialogueSystemRemoteEventHandler,
        QuestEvent = QuestManagementRemoteEventHandler
    },
    SelectedTool = function() return ToolSelectionForFarmingDropdown.Value end
}
