...
local function ExecuteAutomatedQuestFarmingProcedureRoutine()
    ...
    if BasicSettingsToolSelectionDropdownReference and BasicSettingsToolSelectionDropdownReference.Value and BasicSettingsToolSelectionDropdownReference.Value ~= "None Tools" then
        local SelectedToolNameIdentifier = BasicSettingsToolSelectionDropdownReference.Value
        local EquippedToolObjectReference = LocalPlayerReference.Character:FindFirstChild(SelectedToolNameIdentifier) or LocalPlayerReference.Backpack:FindFirstChild(SelectedToolNameIdentifier)
...
local function RetrieveAvailableCharacterToolsInventory()
    if not LocalPlayerReference.Character then return {} end
    local AvailableToolsListContainer = {}
    local ValidToolNamesContainer = {"BlackLeg", "Combat", "Eletric", "WaterKungFu"}
    
    for _, ToolContainerObjectReference in pairs({LocalPlayerReference.Backpack, LocalPlayerReference.Character}) do
        for _, IndividualToolObjectInstance in pairs(ToolContainerObjectReference:GetChildren()) do
            if IndividualToolObjectInstance:IsA("Tool") then
                for _, ValidToolNameIdentifier in pairs(ValidToolNamesContainer) do
                    if IndividualToolObjectInstance.Name:find(ValidToolNameIdentifier) then
                        local alreadyExistsInListValidation = false
                        for _, ExistingToolReference in pairs(AvailableToolsListContainer) do
                            if ExistingToolReference == IndividualToolObjectInstance.Name then
                                alreadyExistsInListValidation = true
                                break
                            end
                        end
                        if not alreadyExistsInListValidation then
                            table.insert(AvailableToolsListContainer, IndividualToolObjectInstance.Name)
                        end
                    end
                end
            end
    ...
    return AvailableToolsListContainer
end

local BasicSettingsToolSelectionDropdownReference = HomeTabContainerReference:AddDropdown("BasicSettingsToolSelection", {Title = "Select Tool Farm", Values = {"None Tools"}, Multi = false, Default = "None Tools"})

HomeTabContainerReference:AddButton({Title = "Update Tools List", Description = "Refresh and update the available tools list with your current inventory", Callback = function()
    local CurrentAvailableToolsListContainer = RetrieveAvailableCharacterToolsInventory()
    if #CurrentAvailableToolsListContainer > 0 then
        BasicSettingsToolSelectionDropdownReference:SetValues(CurrentAvailableToolsListContainer)
    end
end})
...
getgenv().MicaHubMainInterfaceContainer = {
    ...
    SelectedToolReference = function() return BasicSettingsToolSelectionDropdownReference.Value end
}
