...
local StatusUpgradeRoutinesContainer = {
    ...
    Gun = CreateAutomatedStatusUpgradeProcedureRoutine(function() return AutomatedStatusUpgradeStates.Gun end, {Defense = 0, Sword = 0, Gun = 1, Strength = 0, DevilFruit = 0}),
    Strength = CreateAutomatedStatusUpgradeProcedureRoutine(function() return AutomatedStatusUpgradeStates.Strength end, {Defense = 0, Sword = 0, Gun = 0, Strength = 1, DevilFruit = 0}),
    DevilFruit = CreateAutomatedStatusUpgradeProcedureRoutine(function() return AutomatedStatusUpgradeStates.DevilFruit end, {Defense = 0, Sword = 0, Gun = 0, Strength = 0, DevilFruit = 1})
}

for StatusName, RoutineFunction in pairs(StatusUpgradeRoutinesContainer) do
    local ToggleControlReference = StatusTabContainerReference:AddToggle("Automated" .. StatusName .. "StatusUpgrade", {Title = "Auto Status " .. StatusName, Default = false})
    ToggleControlReference:OnChanged(function(ToggleActivationStateValue) 
        AutomatedStatusUpgradeStates[StatusName] = ToggleActivationStateValue
        if ToggleActivationStateValue then 
            task.spawn(RoutineFunction) 
        end 
    end)
end

FruitsTabContainerReference:AddSection("Fruits Functionality")

local AutomatedFruitCollectionToggleControlReference = FruitsTabContainerReference:AddToggle("AutomatedFruitCollection", {Title = "Auto Collect Fruits", Default = false })
...
local AutomatedFruitStorageToggleControlReference = FruitsTabContainerReference:AddToggle("AutomatedFruitStorage", {Title = "Auto Store Fruits", Default = false })
...
TeleportTabContainerReference:AddSection("Teleporting World")
...
local TeleportationDestinationSelectionDropdownReference = TeleportTabContainerReference:AddDropdown("TeleportationDestinationSelection", {Title = "Select Teleport Location", Values = RetrieveAvailablePortalLocationsContainer(), Multi = false, Default = 1})

TeleportTabContainerReference:AddButton({Title = "Teleport to Location [BETA]", Description = "Teleport to Selected Island", Callback = function()
...
MiscTabContainerReference:AddSection("Https Servers Connections")
MiscTabContainerReference:AddParagraph({Title = "How does it work?", Content = "Join servers with specific items using the code available on our Discord."})

local ServerConnectionCodeInputFieldReference = MiscTabContainerReference:AddInput("ServerConnectionCode", {Title = "Target Server Code", Default = "", Placeholder = "", Numeric = false, Finished = false})

MiscTabContainerReference:AddButton({Title = "Join the Server", Description = "Use this button to connect to the server", Callback = function()
...
getgenv().MicaHubMainInterfaceContainer = {
    ToggleStatesContainer = {
        ...
        MaterialFarmingActivation = function() return AutomatedMaterialFarmingActivationState end,
