...
local function ExecuteQuestCancellationProcessForCurrentActiveQuest()
    ...
    QuestManagementRemoteEventHandler:FireServer(unpack(QuestCancellationArgumentsTable))
...
local function ExecuteQuestActivationProcessBySpecificNpcName(SelectedTargetNpcName, QuestDescriptionText)
    ...
    DialogueSystemRemoteEventHandler:FireServer(unpack(QuestActivationArgumentsTable))
    
    AutomatedQuestManagementSystemDatabase[SelectedTargetNpcName].CurrentQuestActivationStatus = true
end

local function RetrieveCurrentPlayerLevelFromUserInterface()
    return tonumber(PlayerLevelDisplayElementReference.Text) or 1
end
...
local function ExecuteAutomatedQuestManagementSystemUpdateBasedOnPlayerLevel()
    ...
    local CurrentPlayerLevelValue = RetrieveCurrentPlayerLevelFromUserInterface()
...
AutomatedQuestFarmingToggleControl:OnChanged(function(ToggleActivationState) 
    ...
    if AutomatedQuestFarmingEnabled then 
        ...
        PlayerLevelDisplayElementReference:GetPropertyChangedSignal("Text"):Connect(ExecuteAutomatedQuestManagementSystemUpdateBasedOnPlayerLevel)
    else
        ExecuteQuestCancellationProcessForCurrentActiveQuest()
    end
end)
